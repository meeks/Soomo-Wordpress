...
class thesis_head {
	...
	function title() {
		global $thesis_site, $thesis_terms;
		...
		if (is_home() || is_front_page()) { #wp
			...
			$home_title = ($thesis_site->home['head']['title']) ? trim(wptexturize(urldecode($thesis_site->home['head']['title']))) : "$site_name $separator $tagline"; #wp
		...
		elseif (is_category()) { #wp
			global $wp_query; #wp
			$category_title = ($thesis_terms->categories[$wp_query->query_vars['cat']]['title']) ? trim(wptexturize(urldecode($thesis_terms->categories[$wp_query->query_vars['cat']]['title']))) : single_cat_title('', false); #wp
			$output = ($thesis_site->head['title']['branded']) ? "$category_title $separator $site_name" : $category_title;
		}
		elseif (is_tag()) {
			global $wp_query; #wp
			$tag_title = ($thesis_terms->tags[$wp_query->query_vars['tag_id']]['title']) ? trim(wptexturize(urldecode($thesis_terms->tags[$wp_query->query_vars['tag_id']]['title']))) : single_tag_title('', false); #wp
	...
	function meta() {
		...
		if (get_option('blog_public') != 0) { #wp
			...
			if (!$page_type && (is_home() || is_front_page())) {
				...
				$robots_meta = ($page_id) ? get_post_meta($page_id, 'thesis_robots', true) : $thesis_site->home['head']['meta']['robots'];
			...
			elseif (is_category()) { #wp
				global $wp_query; #wp
				$cat_noindex = $thesis_terms->categories[$wp_query->query_vars['cat']]['robots']['noindex'];
				$cat_nofollow = $thesis_terms->categories[$wp_query->query_vars['cat']]['robots']['nofollow'];
				$cat_noarchive = $thesis_terms->categories[$wp_query->query_vars['cat']]['robots']['noarchive'];
			...
			elseif (is_tag()) { #wp
				global $wp_query; #wp
				$tag_noindex = $thesis_terms->tags[$wp_query->query_vars['tag_id']]['robots']['noindex'];
				$tag_nofollow = $thesis_terms->tags[$wp_query->query_vars['tag_id']]['robots']['nofollow'];
				$tag_noarchive = $thesis_terms->tags[$wp_query->query_vars['tag_id']]['robots']['noarchive'];
		...
		if (!class_exists('All_in_One_SEO_Pack')) {
			if (is_home() || is_front_page()) {
				...
				$keywords = ($page_id) ? get_post_meta($page_id, 'thesis_keywords', true) : urldecode($thesis_site->home['head']['meta']['keywords']);
				...
				if ($page_id && !get_post_meta($page_id, 'thesis_no_description', true))
					...
					$description = ($thesis_site->home['head']['meta']['description']) ? urldecode($thesis_site->home['head']['meta']['description']) : get_bloginfo('description');
			...
			elseif (is_category() || is_tag()) { #wp
				global $wp_query; #wp
				$description = (is_category()) ? urldecode($thesis_terms->categories[$wp_query->query_vars['cat']]['description']) : urldecode($thesis_terms->tags[$wp_query->query_vars['tag_id']]['description']); #wp
				$keywords = (is_category()) ? urldecode($thesis_terms->categories[$wp_query->query_vars['cat']]['keywords']) : urldecode($thesis_terms->tags[$wp_query->query_vars['tag_id']]['keywords']); #wp
	...
	function scripts() {
		...
		if ((is_single() && get_option('thread_comments')) || (is_page() && get_option('thread_comments') && !$thesis_design->display['comments']['disable_pages']))
