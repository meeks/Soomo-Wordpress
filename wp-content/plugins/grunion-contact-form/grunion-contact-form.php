...
function contact_form_parse( $content ) {
	...
	if ( empty( $_REQUEST['action'] ) ) {
		if ( $_REQUEST['action'] != 'grunion_shortcode_to_json' ) {
...
function contact_form_render_field( $field ) {
	...
	if ( $field['type'] == 'email' ) {
		...
		$r .= "\t<div><label class='". ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(required)") . '</span>' : '' ) . "</label>\n";
