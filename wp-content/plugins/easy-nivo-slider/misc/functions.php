...
?>
	...
	.easy-nivo-slider-widget { 
		...
		overflow:hidden !important;
		<?php echo $options['widget_css'];  ?>
	}
	.easy-nivo-slider-widget .nivo-caption { bottom: <?php 
		echo ('true'==$options['widget_controls_thumbs']) ? 64 : 0; ?>px; }
		
	<?php // ------------------------------------------------------------	?>
	<?php // NEXTGEN SLIDER STYLES											?>
	<?php // ------------------------------------------------------------ 	?>
    <?php if ('true' == get_easy_nivo_slider_option( 'activate_nextgen' )) { ?>		
	.easy-nivo-slider-nextgen { 
		width: <?php echo $options['nextgen_width']; ?>px !important; 
		height: <?php echo $options['nextgen_height']; ?>px !important;
		overflow:hidden !important;
		<?php echo $options['nextgen_css'];  ?>
	}		
		
	.easy-nivo-slider-nextgenwidget { 
		width: <?php echo $options['nextgenwidget_width']; ?>px !important; 
		height: <?php echo $options['nextgenwidget_height']; ?>px !important;
		overflow:hidden !important;
		<?php echo $options['nextgenwidget_css'];  ?>
	}	
	<?php // ------------------------------------------------------------	?>
	<?php // END OF THE NEXTGEN SECTION										?>
	<?php // ------------------------------------------------------------ 	?>
    <?php } ?> 			
...
function sns_set_empty_parms_to_defaults( $parms ) {
	...
	if ('second'!=$parms['size'] && 'widget'!=$parms['size'] && 'nextgen'!=$parms['size'] && 'nextgenwidget'!=$parms['size']) $parms['size'] = 'first';
	$size = $parms['size'];
	...
	$parms['caption_opacity'] 	= $options[ $size.'_caption_opacity' ];
		...
		jQuery(document).ready(function($) {
			$("#slider-<?php echo $parms['ID']; ?>").nivoSlider({
				<?php				
					...
					if ($parms['caption_opacity']) 			echo 'captionOpacity:'.$parms['caption_opacity'].',';
