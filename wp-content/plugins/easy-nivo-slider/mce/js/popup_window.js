...
function generate_shortcode() {
	...
	if ($("#nivo_settings_current_tab").val() == 'tab_featured') {
		...
		if ($("#nivo_orderby").val())			
			shortcode = shortcode + ' orderby="' + $("#nivo_orderby").val() + '"';		
	} 
	...
	// SETTINGS WHICH APPLY TO A CURRENT POST SLIDER
	// ------------------------
	if ($("#nivo_settings_current_tab").val() == 'tab_all') {
		shortcode = shortcode + ' source="all-posts" ';		
	...
	// ------------------------
	// SETTINGS WHICH APPLY TO A SPECIFIC POST SLIDER
	// ------------------------
	if ($("#nivo_settings_current_tab").val() == 'tab_single') {
		
		if ($("#nivo_post_parent").val()!='') {
			shortcode = shortcode + ' source="single-post" post_parent=' + $("#nivo_post_parent").val();
		} else {
			shortcode = shortcode + ' source="all-posts" ';		
		}		
	...
	if ($("#nivo_settings_current_tab").val() == 'tab_nextgen') {
		...
		if ($("#nivo_nextgenorderby").val())			
			shortcode = shortcode + ' nextgenorderby="' + $("#nivo_nextgenorderby").val() + '"';		
	}
	...
	if ($("#nivo_order").val())			
		shortcode = shortcode + ' order="' + $("#nivo_order").val() + '"';		
