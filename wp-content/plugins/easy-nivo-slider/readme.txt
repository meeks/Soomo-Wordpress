...
5. Current post images - create a slider from all the images attached to the single post or page.
...
8. Nivo provides many beautiful transitions to choose from 
...
= 2.0
* Added "Single Post" slider type - specify post/page id for image sore
* Added "Use all images" slider type - pulls from all images on site
* Added "number of images" field to Current Post and NextGen slider types
* Added "page" option to Featured Images slider
* Added "Order By" fields to all sliders - NextGen has different sort options
* Upgraded to version v2.5.1 of Nivo jQuery plugin - adds two new slider transitions, boxRainGrow and boxRainGrowReverse
* Added slider preview to each size's settings panel
* Added "custom css" field for slider 
* Added simple popup help for some of the form fields
* Changed widget behaviour - if slider is empty, don't display anything at all
* Added popup help on several fields
* Changed NextGen slider to use pictures from NextGen as supplied, with fields to crop slider to desired size
* Changed NextGen widget slider to use thumbnails from NextGen as supplied, with fields to crop slider to desired size
* Several bug corrections: eliminate extra slide, remove "debug" message, change "captionOpacity" field name, etc.
...
* Adds "pages" as an option for the featured image slider
* Adds the option to build a slider from a specify post or page
* Adds the option to build a slider from all images uploaded to your website.
* Adds custom styling for the sliders.
* Changes to the options panel, including adding previews to each of the slider pages.
* Uses the updated version v2.5.1 of the Nivo jQuery plugin, which adds two new animation transistions
* Significantly changes the way NextGen galleries are handled.  You can specify crop sizes for NextGen slider and widgets, and 
the slider uses the images and thumbnails maintained by NextGen.  Two new tabs are added to the Settings panel, to allow configuration of the NextGen slider and widget.
